...
if (!projectName) {
    console.log(`
Usage: npm create @mionjs/starter-nuxt <project-name>

Creates a new mion + Nuxt project.
`);
    process.exit(1);
}
...
if (!skipInstall) {
    ...
    try {
        ...
        // `ignoreScripts: true` blocks the project's postinstall too, so
        // run `nuxt prepare` explicitly. The `dev`/`build` scripts also
        // chain it, so this is belt-and-suspenders for first-time setup.
        execSync('pnpm exec nuxt prepare', {cwd: targetDir, stdio: 'inherit'});
    } catch (err) {
        console.error(`
...
supply-chain hardening (minimumReleaseAge, allowBuilds, etc.).
  ...
  • Then re-run:        cd ${projectName} && pnpm install && pnpm exec nuxt prepare
`);
...
console.log(`
Done! Your mion + Nuxt project is ready.
